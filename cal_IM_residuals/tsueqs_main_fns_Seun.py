...
###############################################################################
def compute_baseline(timeseries_stream):
    '''
    ...
    seismic_times = timeseries_stream[0].times()
    
    ## Define the baseline as the mean amplitude in the first 100 samples:
    baseline = np.median(seismic_amp[0:10])#Seun changed the number of samples to 10; it was 100 samples before
...
def highpass(datastream,fcorner,fsample,order,zerophase=True):
    ...
    Input:
        ...
        order:                      The numper of poles in teh filter (use 2 or 4)
